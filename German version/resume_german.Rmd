...
source('common_variables_german.R')  # Small variables for non-csv data such as intro text

# First let's get the data, filtering to only the items tagged as
# Resume items
position_data <- read.csv('positions_Inan_german.csv', sep = ";") %>% 
  filter(in_resume) %>% 
  mutate(
...
# When in export mode the little dots are unaligned, so fix that. 
if(PDF_EXPORT){
  cat(" ") #cat("View this CV online with links at _inan-bostanci.netlify.app_")
} else {
  cat("[<i class='fas fa-download'></i> Download a PDF of this CV](https://github.com/inanimate36/my_cv/raw/Non-website-version/Inan_Bostanci_Resume.pdf)")
}
```
...
Berufliche Erfahrung {data-icon=suitcase}
--------------------------------------------------------------------------------
